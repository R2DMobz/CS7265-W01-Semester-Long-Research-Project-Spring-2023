...
# airline_data = pd.read_csv('files/grouped_airport_data_byMonth.csv')
# covid_data = pd.read_csv('files/grouped_covid_data.csv')

def get_distance(loc1, loc2):
    return hs.haversine(loc1, loc2, unit=Unit.MILES)


def step1_add_closest_airport_to_covid_data():
    covid_data = pd.read_csv('files/grouped_covid_data_lat_lang.csv')
    airline_data = pd.read_csv('files/grouped_airport_data_lat_lang.csv')
    ...
    covid_data.to_csv('files/covid_data_withAirport_Data_v2.csv', index=False)


def step2_transform_airport_to_spike_by_passengers():
    airline_data = pd.read_csv('files/grouped_airport_data_lat_lang.csv')
    airline_data = airline_data.drop(airline_data[(airline_data['Scheduled']  == 0) & (airline_data['Charter'] >= 0)].index)
    ...
    airline_data_pivot.to_csv('files/AirportData_FinalFormatted_Before_Normal.csv')
    # airline_data = airline_data.groupby(['closest_airport', 'closest_airport_lat', 'closest_airport_lng'], axis=0, as_index=False).sum()
    # airline_data.drop(['lat', 'lng', 'closest_airport_distance'], axis=1, inplace=True)

    airline_data_Mod = airline_data_pivot.copy()

    # normalizing the spikes, setting threshold to 50 percent from previous month
    # for i, row in airline_data_pivot.iterrows():
    #     airline_data_Mod.iloc[i, 3] = 0
    #     for j in range(4, airline_data_Mod.shape[1]):
    #         airline_data_Mod.iloc[i, j] = 1 if(row[j - 1] != 0) and ((row[j] - row[j - 1])/row[j - 1]) > THRESHOLD else 0

    airline_data_Mod.to_csv('files/AirportData_for_spike.csv', index=False)


def step3_transform_covid_to_spike_by_airport():
    covid_data = pd.read_csv('files/covid_data_withAirport_Data_v2.csv')
    ...
    covid_data_Mod.to_csv('files/Airports_covid_data_for_spike.csv', index=False)

def step4_merge_covid_airport_data():
    airport = pd.read_csv('files/AirportData_for_spike.csv')
    covid = pd.read_csv('files/Airports_covid_data_for_spike.csv')
    ...
    final_data_frame_2.to_csv('files/FinalTransformedData_ReadyForClassification_v2.csv', index=False)
